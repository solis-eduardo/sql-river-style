/**
 * Motor de renderização de SQL no estilo "river" — profundidade de
 * parênteses/CASE, quebra em cláusulas (SELECT/FROM/JOIN/WHERE/...),
 * CTEs/UNION, CASE em blocos e renderização inline de expressão (ver
 * formatter.ts para a orquestração de topo: entry point, split de
 * statements, e o roteamento pra CREATE FUNCTION/TYPE de plpgsql.ts).
 *
 * Este módulo é a camada compartilhada entre formatter.ts e plpgsql.ts:
 * plpgsql.ts reaproveita `formatQuery`/`renderTokensInline`/
 * `renderExpressionLines`/`uppercaseTypeTokens`/`renderFallbackLines`/
 * `firstMeaningfulKeyword` pra formatar o SQL comum embutido num corpo
 * PL/pgSQL. Nenhum dos dois módulos que consomem este aqui é importado por
 * ele — plpgsql.ts e formatter.ts importam de render.ts, não um do outro
 * diretamente (só formatter.ts -> plpgsql.ts, pra CREATE FUNCTION/TYPE), o
 * que evita o require() circular que existia quando as primitivas
 * compartilhadas viviam dentro de formatter.ts.
 */

import { Token, quoteIdentIfNeeded } from './tokenizer';
import { DepthCursor, matchParen, hasCommaAtDepth0, splitAtCommaDepth0, splitAtSetOpDepth0, stripOuterParens } from './token-scan';
...
export function firstMeaningfulKeyword(tokens: Token[]): string | undefined {
 ...
 */
export function formatQuery(tokens: Token[], indent: number, cfg: Cfg, nested = false): string[] {
  const lines: string[] = [];
  let cursor = 0;
...
export function uppercaseTypeTokens(tokens: Token[]): Token[] {
  const out: Token[] = [];
  let i = 0;
...
export function renderTokensInline(tokens: Token[], cfg: Cfg): string {
 ...
 */
export function renderFallbackLines(tokens: Token[], cfg: Cfg): string[] {
  const lines: string[] = [];
  let segment: Token[] = [];
  for (const t of tokens) {
    if (t.type === 'comment' || t.type === 'blockComment') {
...
function findTopLevelCase(tokens: Token[]): { start: number; end: number } | null {
  ...
  // invisível aqui (não é candidato a `start`), igual ao comportamento de
  // sempre. `cursor.caseDepth` já é o contador de nesting de CASE — não
  // duplica um contador próprio, só lê o dele antes/depois de `advance`.
  const cursor = new DepthCursor();
  let start = -1;

  for (let i = 0; i < tokens.length; i++) {
    const t = tokens[i];
    if (cursor.depth === 0 && t.type === 'keyword' && t.upper === 'CASE' && cursor.caseDepth === 0 && start === -1) {
      start = i;
    }
    cursor.advance(t);
    if (cursor.depth === 0 && t.type === 'keyword' && t.upper === 'END' && cursor.caseDepth === 0 && start !== -1) {
...
function splitCaseBranches(inner: Token[]): { simpleExpr: Token[]; branches: CaseBranch[]; elseExpr: Token[] | null } {
  ...
  // fora de parênteses, e `cursor.caseDepth` (não um contador próprio) diz
  // se um WHEN/THEN/ELSE pertence a um CASE aninhado (caseDepth > 0 depois
  // de processar o token) em vez do CASE mais externo sendo dividido aqui.
  const cursor = new DepthCursor();
  ...
  for (let i = 0; i < inner.length; i++) {
    const t = inner[i];
    cursor.advance(t);
    if (cursor.depth !== 0 || cursor.caseDepth > 0 || t.type !== 'keyword') {
      continue;
    }
 ...
 */
export function renderExpressionLines(tokens: Token[], baseColumn: number, cfg: Cfg): string[] {
